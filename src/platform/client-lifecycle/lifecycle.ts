...
export type ManifestValidation =
  | { ok: true }
  | { ok: false; problems: readonly ManifestProblem[] };
...
export function validateReleaseManifest(
  ...
  if (!manifest.releaseId || !manifest.schemaVersion)
    problems.push("missing_release_identity");
  ...
  for (const asset of manifest.assets) {
    ...
    if (!/^\/[A-Za-z0-9._/-]+$/.test(asset.path) || asset.path.includes(".."))
      problems.push("invalid_asset_path");
    if (!digest.test(asset.sha256)) problems.push("invalid_asset_digest");
    if (!Number.isSafeInteger(asset.size) || asset.size < 0)
      problems.push("invalid_asset_size");
  }
  if (!digest.test(manifest.serviceWorker.sha256))
    problems.push("service_worker_digest_mismatch");
  if (!/^\d+(?:\.\d+)*$/.test(manifest.serviceWorker.version))
    problems.push("invalid_service_worker_version");
  if (
    installedServiceWorkerVersion !== undefined &&
    compareVersions(manifest.serviceWorker.version, installedServiceWorkerVersion) < 0
  ) {
    problems.push("service_worker_downgrade");
  }
  if (
    !manifest.csp.nonceRequired ||
    !manifest.csp.trustedTypesPolicy ||
    !/^[a-zA-Z][a-zA-Z0-9_-]{0,63}$/.test(manifest.csp.trustedTypesPolicy)
  ) {
...
const allowedKey =
  /^(?:hcm-next:client-ref:[A-Za-z0-9._:-]+|hcm-next:ui-preference:[A-Za-z0-9._:-]+)$/;
const sensitiveName =
  /(?:token|secret|password|cookie|salary|medical|bank|ssn|tax|case|document)/i;
...
export function isSafeProtectedReference(
  reference: ProtectedReference,
  now = Date.now(),
): boolean {
  return (
    allowedKey.test(reference.key) &&
    reference.value.length <= 256 &&
    !hasControlCharacter(reference.value) &&
    reference.tenantId.length > 0 &&
    reference.expiresAt > now &&
    !sensitiveName.test(`${reference.key}:${reference.value}`)
  );
}
...
export async function revalidateAction(
  ...
  verify: ActionRevalidator,
): Promise<boolean> {
  if (
    !action.actionId ||
    action.schemaVersion !== expectedSchemaVersion ||
    action.tenantId !== expectedTenantId ||
    !Number.isFinite(action.issuedAt) ||
    (action.expiresAt !== undefined &&
      (!Number.isFinite(action.expiresAt) || action.expiresAt <= Date.now()))
  )
    return false;
...
export class AtomicCacheLifecycle {
  ...
  get snapshot(): CacheState | undefined {
    return this.active;
  }
  stage(
    ...
    const result = validateReleaseManifest(
      manifest,
      trustedKeyIds,
      this.activeServiceWorkerVersion,
    );
    if (
      !result.ok ||
  ...
  clear(): void {
    ...
    this.activeServiceWorkerVersion = undefined;
  }
}
