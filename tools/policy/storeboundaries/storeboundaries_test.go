...
func TestTodo_STORE_002(t *testing.T) {
	...
	wantTenant := map[string]bool{
		...
		"pgstore.go: SELECT ActiveTenants touches tenant-scoped table(s) tenant with no explicit tenant predicate and no tenancy.WithTenant/dbport.Tx signal in package github.com/monstercameron/hcm-next/internal/intent/app/pgstore":                                                                                                                                                                                                   true,
