...
func TestTodo_WEB_046(t *testing.T) {
	...
	if adminRelated == nil || len(adminRelated.Items) != 10 {
		t.Fatalf("roles drawer related = %#v, want Admin parent plus 9 siblings", adminSections)
	}
...
func TestTodo_WEB_046_Golden(t *testing.T) {
	...
	const want = "08aeb2167ca21ada107199d836791865f09a1b428df50a2307e0323ad66e3d77"
	if got != want {
...
func TestTodo_WEB_046_Browser(t *testing.T) {
	...
	if len(collectElements(dialog, "a")) != 10 {
		t.Fatalf("drawer links = %d, want 10 related and no actions on roles page", len(collectElements(dialog, "a")))
	}
