...
func TestTodo_WEB_046(t *testing.T) {
	...
	if adminRelated == nil || len(adminRelated.Items) != 11 {
		t.Fatalf("roles drawer related = %#v, want Admin parent plus 10 siblings", adminSections)
	}
...
func TestTodo_WEB_046_Golden(t *testing.T) {
	...
	const want = "4b38c9b67e0539e0ad09e3de61280197db0cc130151b60d461997084920424f4"
	if got != want {
...
func TestTodo_WEB_046_Browser(t *testing.T) {
	...
	if len(collectElements(dialog, "a")) != 11 {
		t.Fatalf("drawer links = %d, want 11 related and no actions on roles page", len(collectElements(dialog, "a")))
	}
