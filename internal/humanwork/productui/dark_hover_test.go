...
func TestPointerFeedbackUsesSemanticThemeTokens(t *testing.T) {
	...
	for _, want := range []string{
		...
		`.people-workflow-option):hover{background-color:var(--hcm-hover-surface);border-color:var(--hcm-hover-border);color:var(--accent);}`,
		...
		`.button.primary:hover{background-color:var(--accent-hover);color:var(--on-brand);}`,
	} {
...
func TestPointerFeedbackRetainsForcedColorMeaning(t *testing.T) {
	...
	if !strings.Contains(css, want) || !strings.Contains(css, `:hover{background-color:Highlight;border-color:Highlight;color:HighlightText;}`) {
