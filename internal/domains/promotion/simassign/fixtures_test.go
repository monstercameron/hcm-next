...
type memoryOrgFacts struct {
	...
	// skip, when set, is the manager's own manager, which makes the resolved
	// chain two hops deep.
	skip *org.ManagerRelationshipFact
...
func (m *memoryOrgFacts) WorkerFactsAt(_ context.Context, q org.WorkerFactsQuery) (org.WorkerFactSet, error) {
	...
	if m.skip != nil && q.Worker == m.skip.Worker {
		return org.WorkerFactSet{
			Worker: q.Worker, Exists: true,
			Relationships: []org.ManagerRelationshipFact{*m.skip},
			Watermark:     watermark, PolicyVersion: fixturePolicy,
		}, nil
	}
...
// ---------------------------------------------------------------------------
// PROMO-002 simulation fixtures
// ---------------------------------------------------------------------------

// The promotion this lane simulates on top of the PROMO-001 snapshot: Omar
// moves into POS-HRBP-301 (OPS-HRBP3/P3) and starts reporting to a new
// manager. Everything here is intention; nothing here is a current fact.
const (
	fixtureProposedManagerID = "77777777-7777-4777-8777-777777777777"
	fixtureSkipManagerID     = "88888888-8888-4888-8888-888888888888"
	fixtureRevisionID        = "prop_rev_promo002_0001"
	...
	fixtureAuthorityDecision = "PEOPLE_ASSIGNMENT_WRITE_ALLOWED"
	fixtureReservationExpiry = "2026-06-30T00:00:00Z"
	fixtureChainDepthBound   = 3
)
...
func skipManagerRef() values.EntityRef {
	return values.EntityRef{Tenant: fixtures.Tenant, Kind: people.KindWorker, Id: fixtureSkipManagerID}
}

// withSkipLevel makes the org graph answer a two-hop chain: the subject reports
// to the fixture manager, who reports to a skip-level manager. It is the only
// way to exercise a proposed manager that is already inside the resolved chain,
// which is the case in which the depth bound is decidable at all.
func (m *memoryOrgFacts) withSkipLevel(t testing.TB) *memoryOrgFacts {
	t.Helper()
	effective, err := values.NewOpenInstantInterval(mustInstant(t, "2024-04-01T00:00:00Z"))
	if err != nil {
		t.Fatalf("NewOpenInstantInterval: %v", err)
	}
	m.skip = &org.ManagerRelationshipFact{
		RelationshipID: "rel_mgr_2002",
		Type:           org.RelationshipDirectManager,
		Worker:         managerRef(),
		Manager:        skipManagerRef(),
		AssignmentID:   "asg_2002",
		Effective:      effective,
		KnownAt:        mustKnownAt(t, "2024-04-01T00:00:00Z"),
		Revision:       mustRevision(t, "org.relationship.rel_mgr_2002", 2),
		Authority:      fixtureAuthority("hcmnext.org"),
		Provenance:     fixtureProvenance(t, "hcmnext.org", "evd_rel_mgr_2002_r2"),
	}
	return m
}

// fixtureOccupancyFTE is the capacity the subject would take in the target
// position: the whole of it.
func fixtureOccupancyFTE(t testing.TB) values.Decimal {
	t.Helper()
	return mustDecimal(t, "1.0000", 4)
}

// simulateRequest is the PROMO-002 request every test builds from: the
// PROMO-001 snapshot verbatim, plus this promotion's stated intention.
func simulateRequest(t testing.TB, snap promosnapshot.PromotionInputSnapshot) simassign.Request {
	t.Helper()
	return simassign.Request{
		Snapshot: snap,
		...
		ChainDepthBound:    fixtureChainDepthBound,
		ProposalRevisionID: fixtureRevisionID,
		ProposalDigest:     fixtureProposalDigest,
		Occupancy:          simassign.Occupancy{FTE: fixtureOccupancyFTE(t), Heads: 1},
...
func simulate(t testing.TB, h *harness, mutate func(*simassign.Request)) simassign.Result {
	t.Helper()
