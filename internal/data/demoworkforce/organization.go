...
func SeedOrganization(ctx context.Context, tx dbport.Tx, tenant uuid.UUID) (OrganizationSummary, error) {
	...
	legalID := deterministicID("legal-entity", HarborCare.LegalEntity)
	summary := OrganizationSummary{UnitsPlanned: len(HarborCare.Units)}

	legal, err := store.CurrentLegalEntity(ctx, tx, tenant, legalID, effective)
	switch {
	case err == nil:
		if legal.RegisteredName != HarborCare.LegalEntity || legal.LifecycleState != "ACTIVE" {
			return OrganizationSummary{}, fmt.Errorf("demoworkforce: existing HarborCare legal entity conflicts with the deterministic seed")
		}
	case errors.Is(err, aggregates.ErrNotFound):
		candidate, buildErr := aggregates.NewLegalEntity(tenant, legalID, effective, nil, recorded, HarborCare.LegalEntity, "ACTIVE")
		if buildErr != nil {
	...
	unitIDs := make(map[string]uuid.UUID, len(HarborCare.Units))
