...
// the anti-ambiguity framing internal/governance/legal's appendField and
// internal/trust/authz's canonicalRequestDigest both use for their own
// canonical digests: two adjacent fields can never be reparsed as a
...
func appendFields(dst []byte, pairs ...string) []byte {
	if len(pairs)%2 != 0 {
		panic("privacy: appendFields needs an even number of label/value arguments")
	}
...
// appendStringSlice appends a count-prefixed sequence of label-tagged
// fields, preserving the given order. Order is treated as meaningful (a
// notice's disclosed data-class list is a published, ordered disclosure,
// not an unordered set), so a reordering of values changes the digest.
func appendStringSlice(dst []byte, label string, values []string) []byte {
	dst = binary.BigEndian.AppendUint32(dst, uint32(len(values)))
