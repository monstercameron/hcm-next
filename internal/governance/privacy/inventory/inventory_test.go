...
func validInventory() Inventory {
	...
	a := ProcessingActivity{ID: "payroll", Version: "2026-01", Status: StatusApproved, Controller: "acme", Processor: "hcm-next", Purpose: "payroll", DataSubjects: []string{"worker"}, DataCategories: []string{"salary"}, Systems: []string{"hr", "payroll-provider"}, Recipients: []string{"payroll-provider"}, Regions: []string{"US", "EU"}, LawfulBasis: "legal-obligation", Retention: "payroll-7y", SecurityControls: []string{"encryption"}, DPIARef: "dpia/payroll/1", Obligations: []ObligationDeadline{{Kind: MonitoringConsent, Jurisdiction: "EU", RulePackRelease: "eu-2026.1", DeadlineHours: 24, EffectiveFrom: from}, {Kind: BreachNotification, Jurisdiction: "EU", RulePackRelease: "eu-2026.1", DeadlineHours: 72, EffectiveFrom: from}}}
	f := ProcessingDataFlow{ID: "payroll-to-provider", ActivityID: a.ID, Version: a.Version, SourceSystem: "hr", DestinationSystem: "payroll-provider", Recipient: "payroll-provider", Controller: a.Controller, Processor: a.Processor, Purpose: a.Purpose, DataCategories: []string{"salary"}, Operations: []string{"transmit"}, TransferRegions: []string{"EU"}, ContractRefs: []string{"dpa/1"}, Safeguards: []string{"scc"}, SecurityControls: []string{"encryption"}, RetentionRef: a.Retention, EffectiveFrom: from}
