...
func terminalInputs() []workflow.Field {
	return []workflow.Field{{Path: "worker_id", Type: str("WorkerID")}, {Path: "terminal_code", Type: plainStr()}}
}
func terminalMappings(code string) []workflow.Mapping {
	return []workflow.Mapping{{Target: "worker_id", Source: fromInput("worker_id")}, {Target: "terminal_code", Source: constant(code, plainStr())}}
}
func completion(request, execution, business, consistency, obligation string) map[string]string {
	return map[string]string{"RequestState": request, "ExecutionState": execution, "BusinessState": business, "ConsistencyState": consistency, "ObligationState": obligation}
}
	...
	return []workflow.Node{
		{
			...
			InputMappings: []workflow.Mapping{{Target: "worker_id", Source: fromInput("worker_id")}, {Target: "mobility_id", Source: fromInput("mobility_id")}, {Target: "effective_date", Source: fromInput("effective_date")}},
		...
		},
		{
			...
			InputMappings:   []workflow.Mapping{{Target: "mobility_id", Source: fromInput("mobility_id")}, {Target: "source_jurisdiction", Source: fromNode(NodeReadMobilityLegs, "source_jurisdiction")}, {Target: "destination_jurisdiction", Source: fromNode(NodeReadMobilityLegs, "destination_jurisdiction")}},
		...
		},
		{
			...
			InputMappings: []workflow.Mapping{{Target: "mobility_active", Source: fromNode(NodeReadMobilityLegs, "mobility_active")}, {Target: "overlap_detected", Source: fromNode(NodeReadMobilityLegs, "overlap_detected")}, {Target: "source_jurisdiction", Source: fromNode(NodeReadMobilityLegs, "source_jurisdiction")}, {Target: "destination_jurisdiction", Source: fromNode(NodeReadMobilityLegs, "destination_jurisdiction")}, {Target: "home_payroll_group", Source: fromNode(NodeReadMobilityLegs, "home_payroll_group")}, {Target: "host_payroll_group", Source: fromNode(NodeReadMobilityLegs, "host_payroll_group")}, {Target: "payroll_model", Source: fromNode(NodeReadMobilityLegs, "payroll_model")}, {Target: "leg_start", Source: fromNode(NodeReadMobilityLegs, "leg_start")}, {Target: "leg_end", Source: fromNode(NodeReadMobilityLegs, "leg_end")}, {Target: "authorization_valid", Source: fromNode(NodeReadWorkAuthorization, "authorization_valid")}, {Target: "authorization_milestone_complete", Source: fromNode(NodeReadWorkAuthorization, "authorization_milestone_complete")}},
		...
		},
		{
			...
			InputMappings: []workflow.Mapping{{Target: "worker_id", Source: fromInput("worker_id")}, {Target: "mobility_id", Source: fromInput("mobility_id")}, {Target: "footprint_digest", Source: fromNode(NodeComputeFootprint, "footprint_digest")}, {Target: "home_payroll_group", Source: fromNode(NodeReadMobilityLegs, "home_payroll_group")}, {Target: "host_payroll_group", Source: fromNode(NodeReadMobilityLegs, "host_payroll_group")}, {Target: "payroll_model", Source: fromNode(NodeReadMobilityLegs, "payroll_model")}, {Target: "effective_date", Source: fromInput("effective_date")}},
		...
		},
		{
			...
			InputMappings: []workflow.Mapping{{Target: "mobility_active", Source: fromNode(NodeReadMobilityLegs, "mobility_active")}, {Target: "authorization_blocked", Source: fromNode(NodeComputeFootprint, "authorization_blocked")}, {Target: "authorization_milestone_incomplete", Source: fromNode(NodeComputeFootprint, "authorization_milestone_incomplete")}, {Target: "authorization_expiring", Source: fromNode(NodeReadWorkAuthorization, "authorization_expiring")}, {Target: "overlapping_assignment", Source: fromNode(NodeComputeFootprint, "overlapping_assignment")}, {Target: "privacy_transfer_allowed", Source: fromNode(NodeResolvePrivacy, "privacy_transfer_allowed")}, {Target: "tax_impact_unknown", Source: fromNode(NodeResolveTaxPE, "tax_impact_unknown")}, {Target: "pe_impact_unknown", Source: fromNode(NodeResolveTaxPE, "pe_impact_unknown")}},
		...
		end(NodeEndPendingApprovals, "MOBILITY_PENDING_APPROVALS", workflow.RuntimeCompleted, completion("SIMULATED", "NOT_PLANNED", "NOT_STARTED", "PENDING_OBSERVATION", "PENDING"), []string{ObligationDatedLegEvidence, ObligationAuthorization, ObligationTaxPEReview, ObligationPrivacyReview, ObligationVendorObservation, ObligationRecordsRetention}, []string{ApprovalHomePayroll, ApprovalHostPayroll, ApprovalImmigration, ApprovalPrivacy, ApprovalTaxPEReview}),
		end(NodeEndExpiringAuthorization, "MOBILITY_AUTHORIZATION_EXPIRING_REVIEW", workflow.RuntimeCompleted, completion("SIMULATED", "NOT_PLANNED", "NOT_STARTED", "PENDING_OBSERVATION", "PENDING"), []string{ObligationDatedLegEvidence, ObligationAuthorization, ObligationTaxPEReview, ObligationPrivacyReview, ObligationVendorObservation, ObligationRenewalReminder, ObligationRecordsRetention}, []string{ApprovalHomePayroll, ApprovalHostPayroll, ApprovalImmigration, ApprovalPrivacy, ApprovalTaxPEReview}),
		end(NodeEndAuthorizationBlocked, "MOBILITY_AUTHORIZATION_BLOCKED", workflow.RuntimeCompleted, completion("REJECTED", "NOT_PLANNED", "NOT_ACHIEVED", "NOT_APPLICABLE", "PENDING"), []string{ObligationAuthorization, ObligationRecordsRetention}, nil),
		end(NodeEndMilestoneBlocked, "MOBILITY_AUTHORIZATION_MILESTONE_INCOMPLETE", workflow.RuntimeCompleted, completion("REJECTED", "NOT_PLANNED", "NOT_ACHIEVED", "NOT_APPLICABLE", "PENDING"), []string{ObligationAuthorization, ObligationRecordsRetention}, nil),
		end(NodeEndOverlappingLegBlocked, "MOBILITY_OVERLAPPING_ASSIGNMENT_BLOCKED", workflow.RuntimeCompleted, completion("REJECTED", "NOT_PLANNED", "NOT_ACHIEVED", "NOT_APPLICABLE", "PENDING"), []string{ObligationDatedLegEvidence, ObligationRecordsRetention}, nil),
		end(NodeEndPrivacyBlocked, "MOBILITY_PRIVACY_TRANSFER_BLOCKED", workflow.RuntimeCompleted, completion("REJECTED", "NOT_PLANNED", "NOT_ACHIEVED", "NOT_APPLICABLE", "PENDING"), []string{ObligationPrivacyReview, ObligationRecordsRetention}, nil),
		end(NodeEndTaxPEUnknown, "MOBILITY_TAX_PE_IMPACT_UNKNOWN", workflow.RuntimeBlocked, completion("SIMULATED", "BLOCKED", "UNKNOWN", "UNKNOWN", "PENDING"), []string{ObligationTaxPEReview, ObligationRecordsRetention}, nil),
		end(NodeEndDegradedRepair, "MOBILITY_SIMULATION_DEGRADED", workflow.RuntimeBlocked, completion("SIMULATED", "BLOCKED", "UNKNOWN", "UNKNOWN", "PENDING"), []string{ObligationVendorObservation, ObligationRecordsRetention}, nil),
		end(NodeEndUnknown, "MOBILITY_SIMULATION_UNKNOWN", workflow.RuntimeBlocked, completion("SIMULATED", "BLOCKED", "UNKNOWN", "UNKNOWN", "PENDING"), []string{ObligationRecordsRetention}, nil),
	}
}
...
func end(id, code string, status workflow.RuntimeStatus, mapping map[string]string, obligations, approvals []string) workflow.Node {
	return workflow.Node{ID: id, Type: workflow.StepEnd, Inputs: terminalInputs(), InputMappings: terminalMappings(code), End: &workflow.EndSpec{TerminalCode: code, RuntimeStatus: status, CompletionMapping: mapping, OutstandingObligationRefs: obligations, RepairRefs: repairFor(id)}, Governance: terminalGovernance(obligations, approvals)}
}
