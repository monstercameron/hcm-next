...
func DecodeProposalRevisionProjection(payload []byte) (ProposalRevisionProjection, error) {
	...
	return ProposalRevisionProjection{
		...
		DigestAlgorithm: material.GetAlgorithmId(), SchemaRef: fmt.Sprintf("%s@%d", schema.GetSchemaId(), schema.GetVersion()),
		Payload: proposal.GetProtobufWireBytes(), ProducedBy: createdBy.GetPrincipalId(), ProducedAt: createdAt.AsTime(),
	}, nil
}
