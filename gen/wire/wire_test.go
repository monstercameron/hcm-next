...
// TestMarshalStructRoundTrip proves the wire struct adapter round-trips
// nested maps, scalars, nil entries and lists to exact structpb bytes.
func TestMarshalStructRoundTrip(t *testing.T) {
	...
	encoded, err := MarshalStruct(want)
	if err != nil {
		t.Fatalf("MarshalStruct: %v", err)
	}
	round := &structpb.Struct{}
	if err := proto.Unmarshal(encoded, round); err != nil {
		t.Fatalf("unmarshal encoded struct: %v", err)
	}
	if got := round.AsMap(); !reflect.DeepEqual(got, want) {
		t.Errorf("round trip = %#v, want %#v", got, want)
	}
}

// TestMarshalStructRejectsUnsupportedValues proves values structpb cannot
// represent (arbitrary structs like time.Time) surface as an error rather
// than being silently dropped.
func TestMarshalStructRejectsUnsupportedValues(t *testing.T) {
	_, err := MarshalStruct(map[string]any{"bad": time.Time{}})
	if err == nil || !strings.Contains(err.Error(), "invalid type") {
		t.Fatalf("error = %v, want a structpb invalid-type rejection", err)
	}
}
